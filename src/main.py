import sys
from pathlib import Path
from loguru import logger
import polars as pl

# Add src to path for internal imports
sys.path.append(str(Path(__file__).parent))

from utils import setup_logging, init_polars, get_standard_parser, load_and_preprocess
from discovery.engine import FactorDiscoveryEngine


def main():
    """Main entry point for the RiskLens analytical pipeline."""
    parser = get_standard_parser("RiskLens: Factor Discovery Pipeline")
    args = parser.parse_args()

    setup_logging()
    init_polars()

    # 1. Load and Preprocess Data via shared Utility
    df = load_and_preprocess(args.data)

    # 2. Initialize Engine
    engine = FactorDiscoveryEngine(target_column="target")

    # 3. Define features to analyze
    all_features = [col for col in df.columns if col not in ["default", "target"]]

    # 4. Batch Discovery
    logger.info("Executing Batch Factor Discovery Engine...")
    ranking = engine.batch_discovery(df, all_features)

    # 5. Output Findings
    print("\n" + "=" * 50)
    print("RISKLENS: FACTOR DISCOVERY RANKING")
    print("=" * 50)
    with pl.Config(tbl_rows=30):
        print(ranking)
    print("=" * 50)

    # 6. Save results
    output_path = "data/processed/factor_ranking.csv"
    ranking.write_csv(output_path)
    logger.info(f"Results successfully saved to {output_path}")


if __name__ == "__main__":
    main()
